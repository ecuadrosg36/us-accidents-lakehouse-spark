...
from pyspark.sql.functions import (
    col, hour, dayofweek, when, sqrt, abs,
    concat_ws, length, datediff, unix_timestamp
)
...
class FeatureEngineer:
    ...
    def create_location_features(df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn(
            "lat_lng",
            concat_ws(",", col("Start_Lat"), col("Start_Lng"))
        )
    ...
    def create_weather_features(df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn(
            "extreme_temp",
            when(
                (col("Temperature(F)") < 32) | (col("Temperature(F)") > 100),
                1
            ).otherwise(0)
        )
        ...
        df = df.withColumn(
            "poor_visibility",
            when(col("Visibility(mi)") < 2, 1).otherwise(0)
        )
        
        df = df.withColumn(
            "heavy_wind",
            when(col("Wind_Speed(mph)") > 30, 1).otherwise(0)
        )
    ...
    def create_infrastructure_features(df: DataFrame) -> DataFrame:
        ...
        df = df.withColumn(
            ...
            reduce(
                lambda a, b: a + b,
                [coalesce(col(c).cast("int"), lit(0)) for c in infrastructure_cols if c in df.columns]
            )
        )
    ...
    def select_ml_features(df: DataFrame) -> DataFrame:
        ...
        feature_cols = [
            ...
            "Wind_Speed(mph)", "extreme_temp", "poor_visibility", "heavy_wind",
